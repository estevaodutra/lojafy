-- Redefinir import_reference_data_v2 para suportar a importação de múltiplas imagens (galeria de fotos)
...
BEGIN
  ...
  -- Contar especificações manuais (chaves não vazias)
  IF v_product.specifications IS NOT NULL AND jsonb_typeof(v_product.specifications) = 'object' THEN
    SELECT count(*) INTO v_spec_count
    FROM jsonb_each_text(v_product.specifications)
    WHERE value IS NOT NULL AND value <> '';
  END IF;

  -- Contar Marca (brand) como um atributo
  IF COALESCE(v_candidate.brand, v_product.brand) IS NOT NULL AND COALESCE(v_candidate.brand, v_product.brand) <> '' THEN
  ...
  IF v_new_gtin_status IN ('confirmed', 'pending_confirmation', 'not_applicable')
     ...
     AND COALESCE(v_candidate.image_url, v_product.main_image_url, v_product.image_url) IS NOT NULL 
  ...
  SET
    ...
    image_url = CASE WHEN COALESCE((p_overrides->>'apply_image')::boolean, false)
                           THEN COALESCE(v_candidate.image_url, image_url) ELSE image_url END,
    images = CASE WHEN COALESCE((p_overrides->>'apply_image')::boolean, false)
                  THEN COALESCE(
                    (SELECT ARRAY(SELECT jsonb_array_elements_text(v_candidate.raw_data->'pictures'))
                     WHERE v_candidate.raw_data ? 'pictures' AND jsonb_typeof(v_candidate.raw_data->'pictures') = 'array'),
                    ARRAY[v_candidate.image_url]
                  )
                  ELSE images END,
  ...
  SELECT p_product_id, p_candidate_id, v_candidate.ml_item_id, v_imported, v_snapshot_before,
    jsonb_build_object(
      ...
      'images', p.images, 'stage', p.stage, 'reference_item_id', p.reference_item_id
    ), auth.uid()
