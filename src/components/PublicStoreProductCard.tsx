...
const PublicStoreProductCard = ({
  product,
  storeSlug
}: PublicStoreProductCardProps) => {
  const {
    addToFavorites,
    removeFromFavorites,
    isFavorite
  ...
  return <Card className="group hover:shadow-card-hover transition-all duration-300 border-border overflow-hidden h-full max-w-full">
      <CardContent className="p-0 h-full flex flex-col">
        <Link to={`/loja/${storeSlug}/produto/${product.id}`} className="flex-1 flex flex-col">
          <div className="relative overflow-hidden bg-muted/20 aspect-square">
            <img src={product.main_image_url || product.image_url || '/placeholder.svg'} alt={product.name} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300 p-2" />
            {product.badge && <Badge variant={getBadgeVariant(product.badge)} className="absolute top-2 left-2 text-xs max-w-[calc(100%-4rem)]">
                {product.badge}
              </Badge>}
            <Button variant="ghost" size="sm" className="absolute top-2 right-2 bg-white/80 hover:bg-white h-8 w-8 p-0 flex-shrink-0" onClick={handleFavoriteClick}>
          ...
          <div className="p-3 sm:p-4 space-y-2 sm:space-y-3 flex-1 flex flex-col min-w-0">
            <h3 className="font-semibold text-sm sm:text-base text-foreground line-clamp-2 break-words flex items-start gap-2 min-w-0">
              ...
              {product.high_rotation && <span className="text-orange-500 text-sm flex-shrink-0" title="Produto de Alta Rotatividade - Pode haver atraso no envio devido à alta demanda">
                  ⚠️
                </span>}
            </h3>
            ...
            <div className="flex items-center gap-1">
              <div className="flex">
                {[...Array(5)].map((_, i) => <Star key={i} className={`h-3 w-3 sm:h-4 sm:w-4 ${i < Math.floor(Number(product.rating || 0)) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />)}
              </div>
