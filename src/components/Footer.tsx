...
import { Facebook, Instagram, Twitter, Youtube, CreditCard, Shield, Truck, Clock } from "lucide-react";
import { useStoreConfig } from "@/hooks/useStoreConfig";
const Footer = () => {
  const {
    config
  } = useStoreConfig();
  return <footer className="bg-secondary/50 border-t">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Company Info */}
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              {config?.logo_url ? <img src={config.logo_url} alt={config.store_name || "Logo"} className="w-8 h-8 object-contain" /> : <div className="w-8 h-8 bg-hero-gradient rounded-lg flex items-center justify-center">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              {config?.company_address && <div className="flex items-start gap-2">
                  <div className="h-4 w-4 mt-0.5">📍</div>
                  <span>{config.company_address}</span>
                </div>}
              {config?.company_phone && <div className="flex items-center gap-2">
                  <div className="h-4 w-4">📞</div>
                  <a href={`https://wa.me/55${config.company_phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">
                    {config.company_phone}
                  </a>
                </div>}
              {config?.company_email && <div className="flex items-center gap-2">
                  <div className="h-4 w-4">✉️</div>
                  <a href={`mailto:${config.company_email}`} className="hover:text-primary transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {config?.facebook_url && <Button variant="outline" size="sm" asChild>
                  <a href={config.facebook_url} target="_blank" rel="noopener noreferrer">
                    <Facebook className="h-4 w-4" />
                  </a>
                </Button>}
              {config?.instagram_url && <Button variant="outline" size="sm" asChild>
                  <a href={config.instagram_url} target="_blank" rel="noopener noreferrer">
                    <Instagram className="h-4 w-4" />
                  </a>
                </Button>}
              {config?.twitter_url && <Button variant="outline" size="sm" asChild>
                  <a href={config.twitter_url} target="_blank" rel="noopener noreferrer">
                    <Twitter className="h-4 w-4" />
                  </a>
                </Button>}
              {config?.youtube_url && <Button variant="outline" size="sm" asChild>
