...
export function SupplierProductImport({ isOpen, onClose, onSuccess }: SupplierProductImportProps) {
  ...
  const processCSV = (fileContent: string) => {
    ...
    const idxImageUrl = getIndex(['imagem_url', 'imagem', 'url_imagem', 'image_url', 'foto', 'foto_url']);
    ...
    for (let i = 1; i < rawRows.length; i++) {
      ...
      const imageUrlsRaw = idxImageUrl !== -1 ? row[idxImageUrl] || '' : '';
      const imageUrls = imageUrlsRaw
        ? imageUrlsRaw.split(',').map(url => url.trim()).filter(url => url !== '')
        : [];
