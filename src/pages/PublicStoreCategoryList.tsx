...
import { usePublicStoreCategoryProducts, usePublicStoreCategories } from "@/hooks/usePublicStoreProducts";
...
const PublicStoreCategoryList = () => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [sortBy, priceRange, selectedBrand, categorySlug]);

  // Fetch categories
  const { data: categories = [], isLoading: categoriesLoading } = usePublicStoreCategories(store.reseller_id);

  // Fetch products based on category
  const { data: resellerProducts = [], isLoading: productsLoading } = usePublicStoreCategoryProducts(
    store.reseller_id,
    categorySlug
  );
  ...
  // Transform reseller products to match the expected format
  const products = resellerProducts.map((rp: any) => ({
    ...rp.products,
    price: rp.custom_price || rp.products.price,
    description: rp.custom_description || rp.products.description,
  }));
    ...
    >
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
          <Link to={`/loja/${store.store_slug}`} className="hover:text-primary">
            Início
          </Link>
          <ChevronRight className="h-4 w-4" />
          <Link to={`/loja/${store.store_slug}/categorias`} className="hover:text-primary">
            Categorias
          </Link>
          {selectedCategory && (
            <>
            ...
            <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
              <div>
                ...
                <p className="text-muted-foreground">
                  Mostrando {sortedProducts.length > 0 ? startIndex + 1 : 0}-{Math.min(endIndex, sortedProducts.length)}{" "}
                  de {sortedProducts.length} produtos
                </p>
              </div>
                ...
                {paginatedProducts.map((product) => (
                  <Card key={product.id} className="group cursor-pointer hover:shadow-lg transition-shadow">
                    <CardContent className="p-4">
                      ...
                      <Link to={`/loja/${store.store_slug}/produto/${product.id}`}>
                        <Button className="w-full mt-4" style={{ backgroundColor: store.accent_color }}>
                          Comprar
                        </Button>
                      </Link>
                    </CardContent>
...
export default PublicStoreCategoryList;
