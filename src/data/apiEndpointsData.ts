...
const ordersEndpoints: EndpointData[] = [
  {
    ...
    description: 'Atualiza o status de um pedido pelo número do pedido. Suporta 15 status cobrindo todo o ciclo de vida: pagamento, preparação, envio, cancelamento e pós-venda (devolução/troca). Valida transições permitidas entre status. Campos condicionais obrigatórios: cancelamento_motivo (para cancelado), devolucao_motivo (para devolucao_solicitada), troca_motivo (para troca_solicitada), previsao_envio (para em_reposicao).',
    headers: [
    ...
    requestBody: {
      ...
      status: 'cancelado',
      tracking_number: 'BR123456789BR (opcional, útil para enviado)',
      notes: 'Observação geral (opcional)',
      previsao_envio: '2026-02-15 (obrigatório se status = em_reposicao)',
      motivo: 'string (opcional)',
      cancelamento_motivo: 'cliente_desistiu | pagamento_nao_confirmado | estoque | fraude | erro_pedido | outro (obrigatório se status = cancelado)',
      cancelamento_observacao: 'Texto livre (obrigatório se cancelamento_motivo = erro_pedido ou outro)',
      devolucao_motivo: 'produto_defeito | produto_errado | nao_gostou | danificado_transporte | outro (obrigatório se status = devolucao_solicitada)',
      devolucao_observacao: 'Texto livre (opcional)',
      troca_motivo: 'tamanho | cor | produto_defeito | outro (obrigatório se status = troca_solicitada)',
      troca_observacao: 'Texto livre (opcional)'
    },
    responseExample: {
      success: true,
      ...
      _status_disponiveis: 'pendente, pago, recebido, em_preparacao, embalado, enviado, em_reposicao, em_falta, finalizado, cancelado, reembolsado, devolucao_solicitada, em_devolucao, troca_solicitada, em_troca',
      _transicoes: {
    ...
    errorExamples: [
      ...
      { code: 400, title: 'Status inválido', description: 'Status não está na lista dos 15 permitidos', example: { success: false, error: 'Status inválido. Use: pendente, pago, recebido, em_preparacao, embalado, enviado, em_reposicao, em_falta, finalizado, cancelado, reembolsado, devolucao_solicitada, em_devolucao, troca_solicitada, em_troca' } },
      { code: 400, title: 'Transição inválida', description: 'Transição de status não permitida', example: { success: false, error: 'Transição não permitida: pendente → finalizado. Transições válidas: pago, cancelado' } },
      { code: 400, title: 'Motivo cancelamento obrigatório', description: 'cancelamento_motivo é obrigatório quando status = cancelado', example: { success: false, error: 'Campo cancelamento_motivo é obrigatório para status cancelado' } },
      { code: 400, title: 'Observação cancelamento obrigatória', description: 'cancelamento_observacao é obrigatória quando motivo = erro_pedido ou outro', example: { success: false, error: 'Campo cancelamento_observacao é obrigatório quando motivo é erro_pedido ou outro' } },
      { code: 400, title: 'Motivo devolução obrigatório', description: 'devolucao_motivo é obrigatório quando status = devolucao_solicitada', example: { success: false, error: 'Campo devolucao_motivo é obrigatório para status devolucao_solicitada' } },
      { code: 400, title: 'Motivo troca obrigatório', description: 'troca_motivo é obrigatório quando status = troca_solicitada', example: { success: false, error: 'Campo troca_motivo é obrigatório para status troca_solicitada' } },
