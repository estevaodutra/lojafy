...
const ordersEndpoints: EndpointData[] = [
  {
    ...
    description: 'Atualiza o status de um pedido pelo número do pedido. Suporta 15 status cobrindo todo o ciclo de vida: pagamento, recebimento, envio, cancelamento e pós-venda (devolução). Campos condicionais obrigatórios: cancelamento_motivo (para cancelado), devolucao_motivo (para devolucao_andamento), previsao_envio (para em_reposicao), motivo_falta (para em_falta).',
    headers: [
    ...
    requestBody: {
      ...
      motivo_atraso: 'Texto livre (obrigatório se status = em_reposicao)',
      motivo_falta: 'Texto livre (obrigatório se status = em_falta)',
      cancelamento_motivo: 'cliente_desistiu | cliente_recusou_atraso | pagamento_nao_confirmado | estoque_falta | fraude | erro_pedido | solicitacao_revendedor | devolucao_negada | outro (obrigatório se status = cancelado)',
      cancelamento_observacao: 'Texto livre (obrigatório se cancelamento_motivo = erro_pedido, fraude, devolucao_negada ou outro)',
      devolucao_motivo: 'produto_defeito | produto_errado | nao_gostou | danificado_transporte | outro (obrigatório se status = devolucao_andamento)',
    ...
    responseExample: {
      ...
      data: {
        ...
        new_status: 'devolucao_andamento',
        tracking_number: null,
        updated_at: '2026-02-02T12:30:00Z'
      },
      _status_disponiveis: 'pendente, pago, recebido, embalado, enviado, finalizado, em_reposicao, em_falta, cancelamento_solicitado, cancelado, devolucao_andamento, devolucao_recebida, devolucao_analise, devolucao_aprovada, reembolsado',
      _transicoes: {
    ...
    errorExamples: [
      ...
      { code: 400, title: 'Status inválido', description: 'Status não está na lista dos 15 permitidos', example: { success: false, error: 'Status inválido. Use: pendente, pago, recebido, embalado, enviado, finalizado, em_reposicao, em_falta, cancelamento_solicitado, cancelado, devolucao_andamento, devolucao_recebida, devolucao_analise, devolucao_aprovada, reembolsado' } },
      { code: 400, title: 'Motivo cancelamento obrigatório', description: 'cancelamento_motivo é obrigatório quando status = cancelado', example: { success: false, error: 'cancelamento_motivo é obrigatório para status cancelado' } },
      { code: 400, title: 'Observação cancelamento obrigatória', description: 'cancelamento_observacao é obrigatória para certos motivos', example: { success: false, error: 'cancelamento_observacao é obrigatório quando motivo é erro_pedido, fraude, devolucao_negada ou outro' } },
      { code: 400, title: 'Motivo devolução obrigatório', description: 'devolucao_motivo é obrigatório quando status = devolucao_andamento', example: { success: false, error: 'devolucao_motivo é obrigatório para status devolucao_andamento' } },
      { code: 400, title: 'Motivo falta obrigatório', description: 'motivo_falta é obrigatório quando status = em_falta', example: { success: false, error: 'motivo_falta é obrigatório para status em_falta' } },
  ...
  },
  {
    ...
    responseExample: {
      ...
      data: {
        ...
        shipping_status_label: 'Pedido Pago Aguardada Recebimento da Expedição',
        status_envio: 'pago',
        status_envio_label: 'Pedido Pago Aguardada Recebimento da Expedição',
        payment_status: 'paid',
        payment_method: 'pix',
        total_amount: 299.90,
